...
export const Speacialist = async () => {
  const res = await fetch("http://localhost:5000/api/v1/specialites", {
    next: {
      revalidate: 20,
    },
  });
  const specialites = await res.json();
    ...
    <Container>
      ...
      >
        ...
        <Stack direction="row" gap={4} mt={5}>
          {specialites?.data?.slice(2, 12).map((specialty: any) => (
            <Box
              key={specialty.id}
              sx={{
                flex: 1,
